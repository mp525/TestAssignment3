...
public class CustomerStorageImpl implements CustomerStorage {
        ...
        try (var con = getConnection();
            ...
            try (var resultSet = stmt.executeQuery()) {
                if (resultSet.next()){
                    ...
                    return new Customer(id, firstname, lastname, birthdate, phoneNr);
                }
                return null;
            }
        }
    ...
    public List<Customer> getCustomers() throws SQLException {
        try (var con = getConnection();
            ...
            try (ResultSet resultSet = stmt.executeQuery("select ID, firstname, lastname, birthdate, phoneNr from Customers")) {
                while (resultSet.next()) {
                    ...
                    Customer c = new Customer(id, firstname, lastname, birthdate, phoneNr);
                    results.add(c);
                }
            }
    ...
    public int createCustomer(CustomerCreation customerToCreate) throws SQLException {
        var sql = "insert into Customers(firstname, lastname, birthdate, phoneNr) values (?, ?, ?, ?)";
        try (var con = getConnection();
            ...
            stmt.setString(4, customerToCreate.getPhoneNr());
            stmt.executeUpdate();
